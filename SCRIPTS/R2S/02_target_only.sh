...
sed -i '/CONFIG_CPU_FREQ_DEFAULT_GOV/d' target/linux/rockchip/armv8/config-5.15
echo '
...
#Vermagic
latest_version="$(curl -s https://github.com/openwrt/openwrt/tags | grep -Eo "v[0-9\.]+\-*r*c*[0-9]*.tar.gz" | sed -n '/23/p' | sed -n 1p | sed 's/v//g' | sed 's/.tar.gz//g')"
